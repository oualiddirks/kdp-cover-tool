  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <p className="text-sm text-white/50">{loading ? "Loading..." : `${covers.length} cover${covers.length !== 1 ? "s" : ""} created`}</p>
        <Link href="/dashboard/new" className="flex items-center gap-2 rounded-lg bg-violet-600 hover:bg-violet-500 transition-colors px-4 py-2 text-sm font-medium text-white">
      ...
      <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] overflow-hidden">
        {!loading && (
          <div className="hidden md:grid grid-cols-[1fr_140px_120px_100px_100px_100px] gap-4 px-6 py-3 border-b border-white/5 text-xs font-medium text-white/40">
            <span>Cover</span>
            <span>Trim Size</span>
        ...
        {loading ? (
          ...
          <div className="divide-y divide-white/5">
            {covers.map((cover) => (
              <div key={cover.id} className="grid md:grid-cols-[1fr_140px_120px_100px_100px_100px] gap-4 px-6 py-4 items-center hover:bg-white/[0.02] transition-colors">
                <div className="flex items-center gap-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <span className={`inline-flex mt-1 items-center rounded-full px-2 py-0.5 text-[10px] font-medium border ${cover.status === "complete" ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" : "bg-amber-500/10 text-amber-400 border-amber-500/20"}`}>
                      {cover.status}
                    </span>
                  </div>
                </div>
                <div className="hidden md:block"><span className="text-sm text-white/70">{cover.trim_size}"</span></div>
                <div className="hidden md:block"><span className="text-sm font-mono text-violet-300">{cover.spine_width ? `${Number(cover.spine_width).toFixed(4)}"` : "—"}</span></div>
                <div className="hidden md:block">
                  <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium border ${cover.cover_finish === "matte" ? "bg-blue-500/10 text-blue-300 border-blue-500/20" : "bg-amber-500/10 text-amber-300 border-amber-500/20"}`}>
                    {cover.cover_finish}
                  </span>
                </div>
