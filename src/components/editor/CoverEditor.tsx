    ...
    <div className="flex gap-4 w-full">
      ...
      <div className="w-64 shrink-0 flex flex-col gap-3">
        ...
        <div className="flex rounded-lg overflow-hidden border border-white/10 text-xs">
          {(["images","text","background","styles"] as TabId[]).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`flex-1 py-2 capitalize font-medium transition-colors ${activeTab === tab ? "bg-violet-600 text-white" : "bg-white/5 text-white/50 hover:text-white"}`}>
              {tab === "styles" ? "Presets" : tab.charAt(0).toUpperCase() + tab.slice(1)}
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-3 pr-1">
          ...
          {activeTab === "images" && (
            <div className="space-y-3">
              <div>
                <p className="text-xs text-white/40 mb-2">Upload Intent</p>
              ...
              <label className="block w-full rounded-xl border border-dashed border-white/20 hover:border-violet-500/40 bg-white/[0.02] hover:bg-white/[0.04] cursor-pointer p-4 text-center transition-all">
                <svg className="w-6 h-6 mx-auto mb-2 text-white/30" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
          ...
          {activeTab === "text" && (
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                <button onClick={() => setBold(b => !b)} className={`flex-1 py-1.5 rounded-lg text-xs font-bold border transition-all ${bold ? "border-violet-500/60 bg-violet-500/15 text-violet-300" : "border-white/10 bg-white/5 text-white/50"}`}>B</button>
                <button onClick={() => setItalic(i => !i)} className={`flex-1 py-1.5 rounded-lg text-xs italic border transition-all ${italic ? "border-violet-500/60 bg-violet-500/15 text-violet-300" : "border-white/10 bg-white/5 text-white/50"}`}>I</button>
                <div className="flex items-center gap-1 flex-1">
                  <label className="text-[10px] text-white/40">Color</label>
                  <input type="color" value={textColor} onChange={e => setTextColor(e.target.value)} className="w-8 h-7 rounded cursor-pointer border-0 bg-transparent" />
                </div>
              </div>
              ...
              <div>
                ...
                <textarea value={backText} onChange={e => setBackText(e.target.value)} rows={3} placeholder="Enter back cover description..."
              ...
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setAutoSync(v => !v)} className={`w-10 h-5 rounded-full transition-colors relative ${autoSync ? "bg-violet-600" : "bg-white/10"}`}>
            ...
            <div className="grid grid-cols-2 gap-2">
              {PRESETS.map(preset => (
                <button key={preset.id} onClick={() => applyPreset(preset)}
                  ...
                  <div className="h-14 flex items-center justify-center text-xs font-bold"
                    style={{ backgroundColor: preset.bg, color: preset.textColor, fontFamily: preset.font }}>
                    {preset.label}
                  </div>
                  <div className="py-1.5 px-2 text-[10px] text-white/50 group-hover:text-violet-300 bg-white/[0.02] text-center truncate">
                    {preset.label}
                  </div>
                </button>
              ))}
        ...
        <div className="flex items-center gap-2 pt-2 border-t border-white/5">
          ...
          <button onClick={() => { setZoom(z => Math.max(0.3, z - 0.1)); fabricRef.current?.setZoom(Math.max(0.3, zoom - 0.1)); fabricRef.current?.renderAll(); }}
            className="w-7 h-7 rounded-lg border border-white/10 bg-white/5 text-white/60 text-sm hover:bg-white/10 flex items-center justify-center">−</button>
          <span className="text-xs text-white/50 min-w-8 text-center">{Math.round(zoom * 100)}%</span>
          <button onClick={() => { setZoom(z => Math.min(2, z + 0.1)); fabricRef.current?.setZoom(Math.min(2, zoom + 0.1)); fabricRef.current?.renderAll(); }}
            className="w-7 h-7 rounded-lg border border-white/10 bg-white/5 text-white/60 text-sm hover:bg-white/10 flex items-center justify-center">+</button>
          <button onClick={() => { setZoom(1); fabricRef.current?.setZoom(1); fabricRef.current?.renderAll(); }}
            className="ml-auto text-[10px] text-white/30 hover:text-white/60">Reset</button>
        </div>
      </div>

      {/* Canvas area */}
      <div className="flex-1 flex flex-col gap-3 min-w-0">
        <div className="rounded-xl border border-white/[0.08] bg-[#080810] overflow-auto p-3 flex items-start justify-center" style={{ maxHeight: "65vh" }}>
          {!ready && (
            <div className="flex items-center justify-center h-48 text-sm text-white/30 animate-pulse">
              Initialising canvas…
            </div>
          )}
        ...
        <div className="flex gap-3 text-[10px] text-white/40 font-mono">
          ...
          <span>Full W: {fullWidthIn.toFixed(4)}"</span>
          <span>Full H: {fullHeightIn.toFixed(4)}"</span>
          <span>Canvas: {canvasWidthPx}×{canvasHeightPx}px</span>
        </div>

        {/* Preflight */}
        <div className="rounded-xl border border-white/[0.08] bg-white/[0.02] p-3">
          <p className="text-xs font-semibold mb-2 text-white/60">Preflight</p>
          <div className="grid grid-cols-2 gap-1">
            {[
              { ok: preflight.spine,    label: "Spine width valid" },
              { ok: preflight.bleed,    label: "Bleed set correctly" },
              { ok: preflight.textSafe, label: "Text inside safe zone" },
              { ok: preflight.barcode,  label: "Barcode area clear" },
            ].map(({ ok, label }) => (
